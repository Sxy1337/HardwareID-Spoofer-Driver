...
//DONT TOUCH!! - code is explained!
NTSTATUS SMBIOS_SpoofInit() {
	auto* ntoskrnl_base = Utils::ReturnModuleBase(XorString("ntoskrnl.exe")); //Windows program so we'll be able to even spoof
	if (!ntoskrnl_base)
		return STATUS_UNSUCCESSFUL;

	auto smbiosphysicaladdy = static_cast<PPHYSICAL_ADDRESS>(Utils::FindPatternImage(ntoskrnl_base, XorString("\x48\x8B\x0D\x00\x00\x00\x00\x48\x85\xC9\x74\x00\x8B\x15"), XorString("xxx????xxxx?xx"))); // the sig of smbios :D
	if (smbiosphysicaladdy) {
		smbiosphysicaladdy = reinterpret_cast<PPHYSICAL_ADDRESS>(reinterpret_cast<char*>(smbiosphysicaladdy) + 7 + *reinterpret_cast<int*>(reinterpret_cast<char*>(smbiosphysicaladdy) + 3)); //using it here with some calculations
